...
El \textit{backend} implementa una \textbf{Arquitectura en Capas} (\textit{Layered Architecture}) de cuatro niveles:
\begin{enumerate}
...
%\imagenConEtiqueta{img/apdo_4/4_arquitectura_capas.png}{Arquitectura en Capas}{4_arquitectura_capas}
\missingfigure{diagrama arquitectura en capas}
...
Su elección se justifica frente a alternativas como \textit{Thymeleaf}~\cite{thymeleaf:tutorial} por las necesidades específicas del proyecto: la gestión de un elevado número de entidades (productos, pedidos, reservas, usuarios, etc.) y la exigencia de una interfaz dinámica y reactiva. El paradigma basado en \textbf{componentes} y la \textbf{arquitectura SPA} de \textit{React} permiten un desarrollo más modular, mantenible y con mejor experiencia de usuario.
