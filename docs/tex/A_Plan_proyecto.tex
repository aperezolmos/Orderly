...
\begin{itemize}
	...
	\item \textbf{Redacción de apartados restantes de la memoria:} se redactaron el \textit{abstract} y la introducción, junto con los apartados ``\textit{2. Objetivos del proyecto}'' y ``\textit{6. Trabajos relacionados}''. Esto deja pendiente la redacción de conclusiones y algunos aspectos relevantes.
...
\imagenConEtiqueta{img/apen_A/A2_sprint9_burndown.png}{Gráfico \textit{burndown} del Sprint 9}{A2_sprint9_burndown}
...
\imagenConEtiqueta{img/apen_A/A2_sprint10_burndown.png}{Gráfico \textit{burndown} del Sprint 10}{A2_sprint10_burndown}
...
\imagenConEtiqueta{img/apen_A/A2_sprint11_burndown.png}{Gráfico \textit{burndown} del Sprint 11}{A2_sprint11_burndown}
...
Este \textit{sprint} final se basó en la finalización de la redacción de los anexos y en la corrección de aspectos de la aplicación, así como en mejoras de diseño en la interfaz. También se generaron \textbf{datos por defecto} en las tablas de la base de datos, para así contar con registros existentes en el arranque de la aplicación y facilitar su uso.\todo{hacer}

Los \textit{issues} creados para cumplir los objetivos definidos se pueden ver en: \href{https://github.com/aperezolmos/Orderly/milestone/13?closed=1}{\textbf{Sprint 12}}.\todo{cambiar imagen gráfico}

\imagenConEtiqueta{img/apen_A/A2_sprint11_burndown.png}{Gráfico \textit{burndown} del Sprint 12}{A2_sprint12_burndown}

%---------------------------------------------------------------

\subsection{Gráfico \textit{Burn-up}}

El gráfico presentado en la figura \ref{fig:A2_burnup}\todo{cambiar imagen gráfico} permite visualizar de manera dinámica el progreso del trabajo completado frente al alcance total planificado. El eje vertical se corresponde con el número de tareas (\textit{issues}) agregadas al tablero Kanban de \textit{GitHub Projects}.

\imagenConEtiqueta{img/apen_A/A2_sprint11_burndown.png}{Gráfico \textit{burnup} del proyecto}{A2_burnup}
...
\begin{enumerate}
	...
	\[ \frac{600,00\,\text{€}}{48\,\text{meses}} = 12,5\,\text{€/mes} \]
	
	\item Coste total \textit{hardware} (5 meses):
	\[ 12,5\,\text{€} \times 5 = 11.550,00\,\text{€} \]
\end{enumerate}
...
El único coste de software imputable corresponde a la licencia del \textbf{sistema operativo} \textit{Microsoft Windows 11 Home}, con precio aproximado de 145,00€. \todo{se amortiza?}
...
\begin{itemize}
	...
	\item Adicionales:
	\begin{itemize}
		\item \textbf{Dominio} \texttt{.es} $(\text{estimación anual} \approx 12\,\text{€} \rightarrow \text{prorrateado a 5 meses} \approx 12 \times \frac{5}{12} = 5\,\text{€})$.
...
\begin{table}[h]
	...
	\begin{tabularx}{0.8\textwidth}{X r}
		...
		\textit{Software} (Sistema Operativo) & 145,00 \\
		Infraestructura (\textit{Cloud}) & 23,95 \\
		\midrule
		\textbf{Total Neto} & \textbf{11.781,45} \\
		IVA (21 \%) & 2.474,39 \\
		\midrule
		\textbf{TOTAL PROYECTO} & \textbf{14.255,55} \\
		\bottomrule
	\end{tabularx}
\end{table}
