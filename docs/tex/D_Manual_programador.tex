...
En este apartado se detalla la organización del proyecto, el cual sigue una estructura modular que \textbf{separa} claramente las responsabilidades del \textbf{cliente} (\textit{frontend}) y del \textbf{servidor} (\textit{backend}), facilitando así el mantenimiento y la escalabilidad del \textit{software}.
...
\dirtree{%
	...
	.3 sonar-cloud.yml -> Tests de API y reporte a SonarCloud.
...
\dirtree{%
	...
	.2 modules/ -> Funcionalidades de negocio (Pedidos, Productos...).
...
\begin{itemize}
	...
	\item \texttt{hooks}: cada módulo dispone de un \textit{hook} principal (\texttt{use<Entidad>.js}) que centraliza la lógica de comunicación con el \textit{backend}.
...
Más allá de dicha estructura homogénea, cada módulo suele implementar componentes \textbf{propios}, particulares para su funcionamiento. Los directorios \texttt{auth} y \texttt{orders} se alejan un poco de la estructura general, pero mantienen las bases compartidas.
...
Por otro lado, el directorio \texttt{security} contiene la \textbf{infraestructura de seguridad} basada en \textit{Spring Security}, gestionando la autenticación y autorización de la API. Incluye la configuración de \textbf{filtros} de acceso, la definición de usuarios personalizados y servicios específicos para verificar permisos, garantizando que solo los usuarios autenticados y con los permisos adecuados puedan acceder a los \textbf{recursos protegidos}.

Finalmente, el directorio \texttt{init} contiene los archivos de \textbf{inicialización} de la base de datos con registros por defecto, la cual puede activarse o no según una variable de entorno, como se detalla en el apartado \textit{\nameref{subsubsec:D.3_initDefecto}}.

El resto de directorios (exceptuando \texttt{auth}), se corresponden con los \textbf{módulos} de gestión de la aplicación (alimentos, productos, reservas, etc.). Dentro de cada uno se sitúan sus componentes \textbf{específicos}, como archivos de objetos embebidos o tipos enumerados, presentes en los campos de las entidades.

Dejando a un lado las particularidades de cada entidad, todos los directorios de módulos comparten una \textbf{estructura} mínima de archivos y directorios, la cual se puede visualizar en la figura \ref{fig:D2_backend_modules_details}.
...
Esta sección tiene como objetivo profundizar en los aspectos técnicos y las decisiones de diseño que permiten el funcionamiento, la mantenibilidad y la extensibilidad de \textit{Orderly}. Mientras que en el apartado \textit{\ref{sec:D.4_Compilacion} \nameref{sec:D.4_Compilacion}} se detallan los pasos operativos para desplegar el sistema, en esta sección se analiza la arquitectura de \textit{software} desde el punto de vista del desarrollo y la infraestructura.
...
Para el servicio de \textit{Spring Boot} se ha implementado una estrategia de construcción en múltiples etapas (\textit{multi-stage build}). Esta técnica es una buena práctica en ingeniería de \textit{software} que divide el \texttt{Dockerfile} en dos fases diferenciadas~\cite{docker:multistage}:
\begin{enumerate}
...
\label{subsubsec:D.3_reactHMR}

El servicio de \textit{frontend} aprovecha las capacidades de \textit{Vite} para ofrecer una experiencia de \textbf{desarrollo} ágil mediante \textit{HMR} (\textit{\textbf{Hot Module Replacement}}). A diferencia del \textit{Hot Reload} tradicional, que reinicia la aplicación completa ante cualquier cambio, el \textit{HMR} permite \textbf{sustituir} exclusivamente el \textbf{módulo} modificado en tiempo de \textbf{ejecución}. Esto permite al programador visualizar cambios en la interfaz de manera instantánea sin perder el estado actual de la aplicación (por ejemplo, manteniendo modales abiertos o formularios rellenos)~\cite{react:hmr}.
...
\subsubsection{Inicialización de la BBDD con datos de prueba}
\label{subsubsec:D.3_initDefecto}

La inicialización de los roles y el usuario mencionados en el apartado anterior se da \textbf{en todo momento}, durante cada arranque del contenedor \texttt{spring-boot}.

Adicionalmente, se han añadido múltiples \textbf{archivos} de inicialización para el \textbf{resto de entidades} del sistema (alimentos, productos, mesas, etc.), con el objetivo de tener un conjunto de \textbf{datos de prueba} siempre disponible. Dichos archivos se localizan en el directorio \texttt{init} de \textit{Spring Boot} y tienen la particularidad de poder ser \textbf{activados} o desactivados mediante la \textbf{variable} de entorno \texttt{APP\_INIT\_DEMO\_DATA}:
\begin{itemize}
	\item Si su valor es \texttt{true}, se ejecutarán todos los archivos de inicialización, rellenando las tablas con múltiples registros de prueba.
	\begin{itemize}
		\item El \textbf{orden de ejecución} viene dado por la anotación \texttt{@Order}, de manera que las entidades no propietarias son inicializadas \textbf{antes} que las entidades que dependen de otras.
		\item En caso de ya existir registros en una tabla, se \textbf{salta} su inicialización y se procede con el siguiente \textit{script}.
	\end{itemize}
	\item Si su valor es \texttt{false}, ninguno de los archivos se ejecutará. Solo se dispondrá de los \textbf{roles y usuario por defecto} descritos en \textit{\nameref{subsubsec:D.3_rolesUserDefecto}}, los cuales son necesarios para el correcto funcionamiento del sistema.
\end{itemize}

\subsubsection{Consistencia horaria (\textit{Timezone})}

Un aspecto crítico en la gestión de \textbf{reservas} y \textbf{auditoría} de entidades (en particular, de pedidos) es la precisión de las fechas. Por defecto, los contenedores suelen operar en tiempo UTC, lo que causaba \textbf{desajustes} en campos como \texttt{createdAt} o en las horas de reserva respecto a la hora local de Madrid (UTC+1/+2).
...
\begin{tcolorbox}[
	...
	\texttt{MYSQL\_DATABASE=orderly\_db}\\
	...
	\\
	\texttt{\# Application default users configuration}\\
	\texttt{APP\_INIT\_DEMO\_DATA=true}\\
	\\
	\\
	\texttt{\# ======================}\\
	...
	\texttt{REACT\_URL=http://localhost:\$\{REACT\_LOCAL\_PORT\}}\\
	\\
	\\
	\texttt{\# ======================}\\
	\texttt{\# Time Zone}\\
...
\subsection{Permisos de acceso a archivos (\textit{File Sharing})}
\label{subsec:C.3_fileSharing}

Antes de proceder con la ejecución, es importante tener en cuenta que, dependiendo de la configuración de \textit{Docker Desktop} y del sistema operativo anfitrión, puede ser necesario \textbf{autorizar explícitamente el acceso} de \textit{Docker} a la carpeta del proyecto.

Si al intentar levantar los contenedores se produce un \textbf{error} de tipo \textit{\textbf{Mounts denied}} o problemas de permisos al montar los volúmenes, se deben seguir estos pasos para solucionar el conflicto de \textit{File Sharing}~\cite{fileSharing:microsoft,filesharing:dockerForums}:
\begin{enumerate}
	\item Abrir la interfaz de \textbf{\textit{Docker Desktop}}.
	\item Acceder a \texttt{Settings > Resources > File sharing}.
	\item Añadir la \textbf{ruta} del directorio donde se ha clonado el proyecto \textit{Orderly} mediante el botón +.
	\item Hacer clic en \textbf{\textit{Apply} \& \textit{Restart}} para aplicar los cambios.
\end{enumerate}

\textbf{Nota técnica:} este paso es generalmente innecesario en sistemas \textit{Windows 11} que utilicen el motor de \textbf{\textit{WSL 2}}, ya que la gestión de permisos y recursos se realiza de forma automática a través del subsistema de \textit{Linux}. No obstante, es un requisito común en instalaciones que utilicen el \textit{backend} de \textit{\textbf{Hyper-V}} o en sistemas \textit{\textbf{macOS}}.

\subsection{Ejecución de la aplicación}

Una vez configurado el archivo de entorno, el despliegue de los servicios es automático. El flujo de ejecución se rige por el archivo \textbf{\texttt{docker-compose.yml}}, el cual coordina la lectura de los \texttt{Dockerfile} específicos para el \textit{backend} (\textit{Spring Boot}) y el \textit{frontend} (\textit{React}), además de levantar una instancia de \textit{MySQL}.

Para iniciar el proceso de construcción de imágenes y levantado de contenedores, se debe ejecutar el siguiente comando dentro del directorio \texttt{app}:
...
En caso de no poder llevar a cabo la ejecución del comando debido a un \textbf{error} \textit{Mounts denied}, se recomienda seguir los pasos indicados en el apartado \textit{\nameref{subsec:C.3_fileSharing}}.

Por otro lado, si el servicio \texttt{db} mostrara un estado \textit{Unhealthy}, esto indicaría que el \textit{healthcheck} ha \textbf{fallado} al no recibir una respuesta válida de \textit{MySQL} en el tiempo previsto. En este caso, se debe editar el archivo \texttt{docker-compose.yml} y aumentar los valores de \texttt{start\_period} (tiempo de gracia inicial) o \texttt{retries} (número de reintentos), permitiendo así que el motor de base de datos complete su inicialización en equipos con menor rendimiento. Se trata de un escenario poco probable debido a los márgenes de tiempo holgados que se han definido originalmente, aunque factible en sistemas con recursos de \textit{hardware} muy limitados.

\subsection{Acceso al sistema}

Tras finalizar el proceso de arranque (el cual puede demorarse unos minutos en la primera ejecución mientras se descargan las dependencias), la aplicación estará disponible para su uso. Si abrimos \textit{Docker Desktop}, podrán visualizarse los tres contenedores mediante los que se despliega la aplicación (figura \ref{fig:D3_docker}). Haciendo \textit{click} sobre el nombre de cada contenedor, podrán visualizarse sus \textit{logs}, si se quisieran comprobar los detalles del arranque.
...
\begin{itemize}
	...
	\item \textbf{Pruebas de integración}: para las entidades que \textbf{gestionan relaciones} y flujos críticos (por ejemplo, usuarios sobre roles, productos sobre ingredientes, pedidos sobre ítems), se han implementado tests de integración. Estas pruebas se ejecutan sobre una base de datos en memoria (\textbf{H2}) y validan que los procesos completos funcionen correctamente, incluyendo la \textbf{persistencia} y \textbf{consistencia} de las relaciones entre entidades. Así, se comprueba que los cambios realizados en una entidad se reflejan adecuadamente en las entidades relacionadas.
\end{itemize}

El \textit{backend} cuenta con una \textbf{cobertura} de pruebas del 50\%. Aunque no se han cubierto todos los flujos y casos posibles de la API REST, sí se han probado los escenarios más relevantes y \textbf{críticos} para el correcto funcionamiento del sistema, como la gestión de \textbf{relaciones}, la validación de datos y los procesos de negocio principales. La cobertura no era un requisito prioritario, pero se ha procurado asegurar la fiabilidad de los módulos más importantes.
