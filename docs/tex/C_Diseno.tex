...
Los diagramas que se mostrarán a continuación han sido generados mediante la herramienta \textit{``Reverse Engineer''} de \textit{MySQL Workbench}. Cabe señalar que esta funcionalidad \textbf{no soporta} la representación de \textbf{relaciones IS-A}, presentes en la jerarquía de herencia de pedidos (figuras \ref{fig:C2_modeloEER_General} y \ref{fig:C2_modeloEER_Pedidos}). Adicionalmente, las representa con cardinalidad 1:N, lo cual no se corresponde con la cardinalidad real de las herencias con estrategia \textit{JOINED}, pero de nuevo, la herramienta no sabe diferenciar entre 1:1 y 1:N~\cite{stackOverflow:mysqlWorkbench}.

\subsection{Modelo general}
\label{subsec:C.2_ModeloGeneral}

A continuación, se procede a listar las \textbf{entidades} que conforman la aplicación y a describir brevemente su propósito. La figura \ref{fig:C2_modeloEER_General} muestra el esquema completo de la base de datos. Las \textbf{relaciones} entre entidades y detalles particulares se describirán en el apartado \textit{\nameref{subsec:C.2_ModelosDetallados}}.
...
\begin{itemize}
	...
	\end{itemize}
	\item \textbf{Alimento (\wordRef{\texttt{Food}}{C.2_food}):} representa un componente o ingrediente base (p. ej. \textit{``Lechuga''}, \textit{``Pollo''}), con una información nutricional asociada.
	...
	\item \textbf{Pedido (\wordRef{\texttt{Order}}{C.2_order}):} se trata de una comanda que se le hace a un empleado, esto es, uno o varios productos que se piden para ser consumidos y que suman un precio total. Se trata de una entidad ``padre'' que alberga los campos comunes entre tipos de pedidos. Se verá con mayor detalle en la sección \textit{\nameref{subsubsec:C.2_ModeloPedidos}}.
	...
	\item \textbf{Línea de pedido (\wordRef{\texttt{OrderItem}}{C.2_orderItem}):} representa un ítem de cada pedido, compuesto por un producto y una cantidad de dicho producto. Constituye un porcentaje del precio total del pedido.
\end{itemize}
...
\label{subsubsec:C.2_ModeloPedidos}
...
La seguridad del sistema se ha diseñado bajo un modelo de \textbf{control de acceso basado en roles} (\textit{RBAC})~\cite{ibm:rbac} de \textbf{granularidad fina}. A diferencia de modelos más simples donde el rol determina directamente el acceso, en esta arquitectura los roles actúan como \textbf{contenedores} lógicos de permisos individuales. Estos permisos, definidos en el enumerado \hyperref[word:C.2_permission]{\texttt{Permission}}, representan acciones atómicas sobre las entidades del dominio (ej. \texttt{FOOD\_CREATE}, \texttt{ORDER\_EDIT}, \texttt{USER\_VIEW\_LIST}).
...
\begin{itemize}
	...
	\item \textbf{Extensibilidad:} facilita la \textbf{sustitución} o adición de \textbf{nuevos proveedores} de datos nutricionales con un impacto mínimo en el resto del sistema, ya que el \textbf{contrato} de datos que recibe el \textit{frontend} permanece \textbf{inalterado}. Esta cuestión se explora más a fondo en el apartado \textit{\nameref{subsec:D.3_nuevaAPI}}.
\end{itemize}
...
La infraestructura de la aplicación se gestiona mediante \textit{Docker}, empleando \textit{\textbf{Docker Compose}} para la orquestación de los servicios. El sistema se descompone en \textbf{tres contenedores independientes}: el entorno de ejecución para el \textit{frontend} (\textit{React/Vite}), el servidor de aplicaciones (\textit{Spring Boot}) y el motor de base de datos (\textit{MySQL}). 

Durante la fase de \textbf{desarrollo}, la infraestructura se configura para maximizar la productividad del programador. Como se observa en el diagrama \ref{fig:C3_despliegue_dev}, se emplean \textbf{volúmenes de datos} específicos en el servicio \textit{frontend} para habilitar el \textit{Hot Module Replacement} (HMR). 

Esta configuración permite que los cambios en el código fuente se reflejen instantáneamente en el navegador sin necesidad de reconstruir las imágenes. Los detalles técnicos sobre el funcionamiento de estos volúmenes y su relación con el flujo de trabajo se detallan en el apartado \textit{\nameref{subsubsec:D.3_reactHMR}}.

\imagenConEtiqueta[0.8\textwidth]{img/apen_C/C3_despliegue_dev.png}{Diagrama de despliegue en entorno de desarrollo}{C3_despliegue_dev}

Para el entorno de \textbf{producción}, la arquitectura se simplificaría para garantizar la portabilidad y el rendimiento. En este escenario (diagrama \ref{fig:C3_despliegue_prod}):
\begin{itemize}
	\item \textbf{Eliminación de dependencias locales:} se prescindiría de los volúmenes del \textit{frontend}. El código fuente y las dependencias quedarían embebidos estáticamente dentro de la imagen del contenedor, eliminando la dependencia con el sistema de archivos del anfitrión.
	\item \textbf{Persistencia de datos:} se mantendría el volumen persistente \break \texttt{orderly\_db\_data} para el servicio \textit{MySQL}. Esto asegura que la información de la base de datos sea independiente del ciclo de vida del contenedor.
\end{itemize}

\imagenConEtiqueta[0.8\textwidth]{img/apen_C/C3_despliegue_prod.png}{Diagrama de despliegue en entorno de producción}{C3_despliegue_prod}
...
\imagenConEtiqueta[0.9\textwidth]{img/apen_C/C4_orderMapperFactory.png}{Implementación del patrón \textit{Factory} para la gestión polimórfica de pedidos y sus DTOs}{C4_orderMapperFactory}
...
\imagenConEtiqueta[0.9\textwidth]{img/apen_C/C4_metodosDominio.png}{Ejemplo de métodos de dominio en entidades \texttt{Product}, \texttt{Food} y \texttt{Order}}{C4_metodosDominio}
...
\imagenConEtiqueta[0.9\textwidth]{img/apen_C/C4_metodosConsistencia.png}{Ejemplos de métodos para consistencia bidireccional en entidades \texttt{Ingredient}, \texttt{Order} y \texttt{OrderItem}}{C4_metodosConsistencia}
...
{\small
\begin{enumerate}
	\item Empleado accede al formulario de creación en la aplicación \textit{React} y lo rellena.
