...
\begin{itemize}
	...
	\req{RNF-6 Interoperabilidad y consumo de API externa:}{rnf6} la integración con una API externa de terceros (\hyperref[req:rf1.1.2]{RF-1.1.2}, \hyperref[req:rf9]{RF-9}) debe ser robusta y manejar adecuadamente los fallos de la fuente externa.
...
Es importante destacar que la aplicación implementa un \textbf{sistema de control de acceso basado en roles (\textit{RBAC})}\footnote{\textit{Role-Based Access Control}}~\cite{ibm:rbac}, el cual permite crear configuraciones personalizadas que materialicen roles funcionales más específicos, como por ejemplo \textit{``Cocinero''} o \textit{``Camarero de barra''}. Para simplificar el diagrama \ref{fig:B.4_casos_uso}, no se modelan como actores separados, pero son completamente realizables mediante el módulo de gestión de roles mencionado. Se profundiza más en esta característica del sistema en el apartado \textit{\nameref{subsec:C.3_modeloRBAC}}.
...
\BeginCasoUso{tab:CU-1}{CU-1}{Registrarse (\textit{register})}
	...
	\begin{enumerate}[nosep]
		...
		{\footnotesize
		\begin{enumerate}[nosep]
			\item Si la validación es exitosa, el sistema crea el usuario y le añade el rol por defecto. Se inicia sesión automáticamente con el nuevo usuario registrado, redirigiendo a la página de inicio.
...
\BeginCasoUso{tab:CU-2}{CU-2}{Iniciar sesión (\textit{login})}
	...
	\begin{enumerate}[nosep]
		...
		{\footnotesize
		\begin{enumerate}[nosep]
			\item Si la validación es exitosa, el sistema redirige al usuario a la página de inicio.
			\item Si la validación falla, se muestra un mensaje con la descripción del error.
		\end{enumerate}}
	\end{enumerate} \\
	\textbf{Postcondición}        & 
...
\BeginCasoUso{tab:CU-4}{CU-4}{Editar perfil propio}
	...
	\begin{enumerate}[nosep]
		...
		{\footnotesize
		\begin{itemize}[nosep]
			\item Si se ha modificado el \textit{username}, se cerrará sesión para que el usuario vuelva a ingresar con las nuevas credenciales.
		\end{itemize}}
	\end{enumerate} \\
...
\begin{enumerate}[nosep]
	...
	\item El sistema valida la transición de estado.
	{\footnotesize
	\begin{enumerate}[nosep]
		\item Si la la transición es válida, persiste los cambios y actualiza la vista.
		\item Si falla, muestra un mensaje de error.
	\end{enumerate}}
\end{enumerate} \\
\textbf{Postcondición}        & 
\begin{itemize}[nosep]
...
\textbf{Actor}                & Empleado con permisos \texttt{EDIT} sobre pedidos y \texttt{PRODUCT\_VIEW\_LIST}. \\
...
\begin{itemize}[nosep]
	...
	\item Está en el formulario de creación o edición de un usuario.
\end{itemize} \\
\textbf{Acciones}             &
\begin{enumerate}[nosep]
	\def\labelenumi{\arabic{enumi}.}
...
\begin{itemize}[nosep]
	...
	\item En el botón \textit{Filtrar} aparece el número de alérgenos que se están excluyendo.
\end{itemize} \\
\textbf{Excepciones}          &
\begin{itemize}[nosep]
