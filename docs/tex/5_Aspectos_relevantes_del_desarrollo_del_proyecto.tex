...
\todo[inline]{spring boot + mysql + docker -> sistemas distribuidos asignatura + ventajas que ofrecen
...
\todo[inline]{arquitectura de la aplicación?? introducción + diagrama y luego completar en anexos -> o hacer una sección propia ANTERIOR a esta?}
...
\begin{itemize}
	...
	\item \textbf{Gestión de pedidos (comandas):} Soporta la creación y seguimiento de pedidos realizados en el local, diferenciando entre pedidos de barra y de comedor mediante una jerarquía de herencia.\todo{referenciar anexo diciendo que se explica ahí la jerarquía?}
...
Cada módulo está implementado siguiendo el patrón de capas\todo{ref a apartado o anexo que lo explique}, con entidades de dominio, repositorios, servicios y controladores, lo que facilita la mantenibilidad y la extensibilidad del sistema.
...
\todo[inline]{Métodos de consistencia bidireccional y campos de auditoría -> aquí o en el apéndice C: Diseño de datos??}
...
La seguridad y el control de acceso en el \textit{backend} se gestionan mediante \textit{Spring Security}, implementando un sistema de autenticación basado en sesiones HTTP y \textbf{autorización granular por roles y permisos}. Los \textit{endpoints} de la API están protegidos mediante anotaciones como \texttt{\char`@{}PreAuthorize}, que verifican los permisos necesarios para cada operación, garantizando que solo los usuarios autorizados puedan acceder a los recursos sensibles.
...
\todo[inline]{Después aquí explicar MUY BREVEMENTE los endpoints utilizados o los datos extraídos de la api. -> o hacer esto en apartado 4. Técnicas y herramientas?}
